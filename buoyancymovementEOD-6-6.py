...
def plunger_down():
	...
	if GPIO.input(switch) == False: #check if it's true by software protection
		...
		if GPIO.input(switch) == False: #check if it's true by software protection
			volts = get_battery()
			if volts >= 5.8: plunger_time = 65
			if volts < 5.8: plunger_time = 68
			if volts < 5.7: plunger_time = 72
			if volts < 5.6: plunger_time = 77
			if volts < 5.5: plunger_time = 82
			if volts < 5.3: plunger_time = 90
			print("volts:" + str(volts) + " time:" + str(plunger_time))
...
#	print("Is power plugged: %s" f% (battery_status.power_plugged,))
	time.sleep(4)
	pot = MCP3008(0)
