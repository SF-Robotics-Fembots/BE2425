...
def Go_To_Depth(target_depth):
	...
	while (1):
		...
		if (0.5 > (depth_diff) > 0.3):
			print("depth diff: ", depth_diff)
			print("depth diff: ", depth_diff, file=file2)
			Go_To_Pos(SYRINGE_NEUTRAL + 0.75)
			print("going to downwards position: ", SYRINGE_NEUTRAL + 0.75)
			print("going to downwards position: ", SYRINGE_NEUTRAL + 0.75, file=file2)
		if ((depth_diff) > 0.5):
			print("depth diff: ", depth_diff)
			print("depth diff: ", depth_diff, file=file2)
			Go_To_Pos(SYRINGE_NEUTRAL + 1.5)
			print("going to downwards position: ", SYRINGE_NEUTRAL + 1.5)
			print("going to downwards position: ", SYRINGE_NEUTRAL + 1.5, file=file2)				
	#if reached target
		...
		if (-0.3 <= (depth_diff) < -0.10):
			print("depth diff: ", depth_diff)
			print("depth diff: ", depth_diff, file=file2)
			Go_To_Pos(SYRINGE_NEUTRAL - 0.25)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.25)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.25, file=file2)
		if (-0.5 <= (depth_diff) < -0.3):
			print("depth diff: ", depth_diff)
			print("depth diff: ", depth_diff, file=file2)
			Go_To_Pos(SYRINGE_NEUTRAL - 0.5)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.5)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.5, file=file2)
		if ((depth_diff) <= -0.5):
			print("depth diff: ", depth_diff)
			print("depth diff: ", depth_diff, file=file2)
			Go_To_Pos(SYRINGE_NEUTRAL - 0.75)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.75)
			print("going to upwards position: ", SYRINGE_NEUTRAL - 0.75, file=file2)	
def init_html():
...
def readings(counter):
        ...
        while second_time <= counter:
                ...
                if depth >= 5 or depth <-0.35:
                        continue
                depth =  depth * -1
                print("RN08" + " : " + (str(second_time)) + " : " + (str(pressure)) + " : " + (str(depth)), file=k)
                print("RN08" + " : " + (str(second_time)) + " : " + (str(pressure)) + " : " + (str(depth)))
